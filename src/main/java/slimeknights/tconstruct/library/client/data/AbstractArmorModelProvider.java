...
public abstract class AbstractArmorModelProvider extends GenericDataProvider {
  ...
  public CompletableFuture<?> run(CachedOutput output) {
    ...
    // Convert each save task to a CompletableFuture and collect them
    List<CompletableFuture<Void>> futures = models.entrySet().stream()
      .map(entry -> CompletableFuture.runAsync(() -> {
        saveJson(output, entry.getKey(), ArmorModel.LOADABLE.serialize(entry.getValue()));
      }))
      .toList();
