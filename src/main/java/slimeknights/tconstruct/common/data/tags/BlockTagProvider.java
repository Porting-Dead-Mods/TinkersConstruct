...
public class BlockTagProvider extends VanillaBlockTagsProvider {
  ...
  private void addHarvest() {
    // commons
    tagBlocks(MINEABLE_WITH_SHOVEL, TinkerCommons.mu);
    ...
    for (SlimeType dirt : SlimeType.values()) {
      for (SlimeType grass : SlimeType.values()) {
        Tiers dirtTier = dirt.getDirtType().getHarvestTier();
        Tiers grassTier = grass.getDirtType().getHarvestTier();
        ...
        this.tag(Objects.requireNonNull(tier.getTag())).add(TinkerWorld.slimeGrass.get(dirt).get(grass.asFoliage()));
      }
    }
    ...
    tag(TinkerTags.Blocks.MINABLE_WITH_SHEARS)
      ...
      .addTag(BlockTags.CAVE_VINES).addTag(BlockTags.LEAVES).addTag(BlockTags.WOOL).addTag(BlockTags.SAPLINGS).addTag(BlockTags.FLOWERS).addTag(BlockTags.REPLACEABLE).addTag(BlockTags.CORAL_PLANTS);
    // scythe/kama does hoe or shear blocks
