...
public class GeodeItemObject extends ItemObject<Item> {
  ...
  public GeodeItemObject(RegistryObject<? extends Item> shard, BlockDeferredRegister register, MapColor color, SoundType blockSound, SoundEvent chimeSound, Map<BudSize,SoundType> clusterSounds, int baseLight, Properties props) {
    ...
    block = register.register(name + "_block", () -> new CrystalBlock(chimeSound, BlockBehaviour.Properties.copy(Blocks.AMETHYST_BLOCK).lightLevel(crystalLight).strength(1.5F).sound(blockSound).requiresCorrectToolForDrops()), blockItem);
    budding = register.register("budding_" + name, () -> new BuddingCrystalBlock(this, chimeSound, BlockBehaviour.Properties.copy(Blocks.BUDDING_AMETHYST).randomTicks().lightLevel(crystalLight).strength(1.5F).sound(blockSound).requiresCorrectToolForDrops()), blockItem);
    // buds
    Supplier<BlockBehaviour.Properties> budProps = () -> BlockBehaviour.Properties.copy(Blocks.LARGE_AMETHYST_BUD).noOcclusion().strength(1.5F);
